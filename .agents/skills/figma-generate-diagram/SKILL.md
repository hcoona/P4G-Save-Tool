...
Mermaid can't do everything. Sticky-note annotations tied to specific nodes, per-node domain coloring on ERDs, callouts with attached data — these all require composing `generate_diagram` with `use_figma` (via the [figma-use-figjam](../../../apm_modules/figma/mcp-server-guide/skills/figma-use-figjam/SKILL.md) skill). This is the **hybrid workflow**.
