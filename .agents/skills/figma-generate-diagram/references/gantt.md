...
Instead, build the timeline directly on a FigJam canvas using the `use_figma` tool. The [figma-use](../../../../apm_modules/figma/mcp-server-guide/skills/figma-use/SKILL.md) and [figma-use-figjam](../../../../apm_modules/figma/mcp-server-guide/skills/figma-use-figjam/SKILL.md) skills cover how to: create a new FigJam file, place shapes and connectors, position nodes on a time axis, add sticky notes and annotations, color-code elements, and group content into sections. Load those skills and compose the timeline to the user's actual spec.
