...
- **Follow the [canonical text-edit recipe](../../../../apm_modules/figma/mcp-server-guide/skills/figma-use/references/gotchas.md#canonical-text-edit-recipe-font-load--await--mutate--return-ids)** for `shape.text.characters` — always load `shape.text.fontName` (ShapeWithText defaults to `Inter Medium`, not Regular); never hardcode the family/style.
