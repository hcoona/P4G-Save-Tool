...
- **Follow the [canonical text-edit recipe](../../../../apm_modules/figma/mcp-server-guide/skills/figma-use/references/gotchas.md#canonical-text-edit-recipe-font-load--await--mutate--return-ids)** for `cell.text.characters` and `cell.text.fills` — load `cell.text.fontName` (use the first cell’s, or each cell’s if they differ).
