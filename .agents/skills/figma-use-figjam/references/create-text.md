...
- **Follow the [canonical text-edit recipe](../../../../apm_modules/figma/mcp-server-guide/skills/figma-use/references/gotchas.md#canonical-text-edit-recipe-font-load--await--mutate--return-ids)** for `characters`, `fontSize`, `fontName`, or any property that affects layout; not required for `fills` (color) only.
