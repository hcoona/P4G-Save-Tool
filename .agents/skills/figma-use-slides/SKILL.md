...
This skill contains Slides-specific context for the `use_figma` MCP tool. The [figma-use](../../../apm_modules/figma/mcp-server-guide/skills/figma-use/SKILL.md) skill provides foundational context for plugin API execution via MCP as well as the full Figma plugin API for more advanced use-cases that are not described here.
...
When building a new deck of 5 or more slides, use this two-phase workflow. It replaces the general incremental workflow from [figma-use](../../../apm_modules/figma/mcp-server-guide/skills/figma-use/SKILL.md) Section 6 for deck-building specifically — the principles still apply, but the cadence changes.
...
- **Inspect before creating.** Before creating anything, run a read-only `use_figma` to discover what already exists — slides, text, components, naming conventions. The [figma-use](../../../apm_modules/figma/mcp-server-guide/skills/figma-use/SKILL.md) Section 6 "Inspect first" pattern applies here.
